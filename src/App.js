...
function App() {
  ...
  const contractAddress = "0xeB2887696e00c069859c96F1ab65478fa259e2Ad"; // Paste the address recieved from Remix IDE here
...
const [profilePhoto, setProfilePhoto] = useState(null);
  ...
  const handleProfilePhotoChange = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setProfilePhoto(reader.result); // base64 image
      };
  ...
  const loadTotalPeople = async () => {
    try {
      const count = await contract.getTotalPeople();
      setTotalPeople(count.toNumber());
    } catch (error) {
      console.error("Error loading total people:", error);
    }
  };
...
<div style={{ position: "absolute",
  ...
  <input
    ...
    style={{ display: "none" }}
  />
  <p style={{ fontSize: "0.9em", color: "white" }}>Upload photo</p>
</div>
...
{/* No of registered User */}
  <h3>Total Registered Users: {totalPeople}</h3>
</div>
